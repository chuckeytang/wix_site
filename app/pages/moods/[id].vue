      ...
      <template v-else-if="moodDetail">
        <div class="breadcrumb-container">
          <div class="container">
            ...
            <span>></span>
            <span class="breadcrumb-item current">{{ moodDetail.name }}</span>
          </div>
        </div>
...
const moodId = ref(route.params.id);

const moodDetail = ref<Moods | null>(null);
...
const fetchData = async () => {
  ...
  try {
    const moodResponse = await moodsApi.getMoodDetail(id);
    if (moodResponse.code === 200 && moodResponse.data) {
      moodDetail.value = moodResponse.data as Moods;
    } else {
